"""
MH-2026-006 工程圖紙文字區域偵測 — YOLOv8s fine-tune（本機 CPU 版）

目標：mAP50 >= 0.70（baseline >= 0.60）
資料：360 張 text_detection + auto-labels (1 class: text)
設備：CPU（MPS 有 YOLO validation bug）
"""
import os
import json
import shutil
...
REQ_NO = "MH-2026-006"
DATA_ROOT = Path("/Users/yinghaowang/HurricaneCore/docker-data/modelhub/datasets/text_detection")
...
SEED = 42
VAL_RATIO = 0.2
EPOCHS = int(os.getenv("MH006_EPOCHS", "50"))
...
print(f"[PAIRS] {len(pairs)} image-label pairs", flush=True)
