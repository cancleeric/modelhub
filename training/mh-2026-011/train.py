"""
MH-2026-011 手繪工程圖品質路由分類器 — EfficientNet-B0（本機 Mac MPS 版）

目標：val_accuracy >= 0.90
資料：4 tier × 400 imgs @ ~/HurricaneCore/docker-data/modelhub/datasets/quality_router
...
REQ_NO = "MH-2026-011"
DATA_DIR = Path("/Users/yinghaowang/HurricaneCore/docker-data/modelhub/datasets/quality_router")
OUT_DIR = Path(f"/Users/yinghaowang/HurricaneCore/modelhub/training/{REQ_NO.lower()}")
BEST_PATH = OUT_DIR / "quality_router_best.pth"
...
NUM_CLASSES = 4
EPOCHS = int(os.getenv("MH011_EPOCHS", "30"))
BATCH_SIZE = int(os.getenv("MH011_BATCH", "32"))
LR = 1e-3
IMG_SIZE = 224
SEED = 42
...
print(f"[MODEL] loading efficientnet_b0 (pretrained ImageNet)")
...
if best_acc >= 0.90:
    ...
    verdict, tier = "baseline", "baseline 可交付（val_acc >= 0.85 但未達 0.90）"
else:
